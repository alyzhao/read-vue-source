/**
 * 挂载真实节点
 * @param{Element} oldVnode 这是之前的节点, 初次渲染时就是 选项 el
 */
export function patch (oldVnode, vnode) {
  ...
  if (oldVnode.nodeType === 1) {
    ...
    const parentNode = oldVnode.parentNode; // https://developer.mozilla.org/zh-CN/docs/Web/API/Node/parentNode 获取父节点
    parentNode.insertBefore(el, oldVnode.nextSibling); // 插入节点
    ...
    if (oldVnode.tag !== vnode.tag) {
      ...
      if (oldVnode.text !== vnode.text) {
        oldEl.textContent = vnode.text;
      }
      return oldEl;
    } else {
      // 标签一样需要比较属性和子元素, 此时复用老节点
      vnode.el = oldVnode.el;
      const el = oldVnode.el; // 此时当前虚拟节点的el用原来虚拟节点的el即可
